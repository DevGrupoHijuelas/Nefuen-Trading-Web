...
function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    if (progress < 0.333) {
      ...
      angle = THREE.MathUtils.lerp(0, isMobile ? Math.PI * 0.4 : Math.PI * 0.5, ease)
      radius = THREE.MathUtils.lerp(15, isMobile ? 7 : 8, ease)
      y = THREE.MathUtils.lerp(5, isMobile ? 1.5 : 2, ease)
      lookX = THREE.MathUtils.lerp(0, isMobile ? 2 : 3, ease)
      lookY = THREE.MathUtils.lerp(0, 0, ease)
    } else if (progress < 0.666) {
      const t = (progress - 0.333) / 0.333
      const ease = t * t * (3 - 2 * t)
      angle = THREE.MathUtils.lerp(isMobile ? Math.PI * 0.4 : Math.PI * 0.5, Math.PI, ease)
      radius = THREE.MathUtils.lerp(isMobile ? 7 : 8, isMobile ? 6 : 6, ease)
      y = THREE.MathUtils.lerp(isMobile ? 1.5 : 2, 1, ease)
